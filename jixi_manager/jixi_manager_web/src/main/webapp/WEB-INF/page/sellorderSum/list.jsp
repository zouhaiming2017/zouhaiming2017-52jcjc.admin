...
<head>
    ...
    <![endif]-->
    <title>客户订单汇总列表</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>客户订单汇总列表管理 <span class="c-gray en">&gt;</span> 客户订单汇总列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <div class="text-c">
        <form action="/sellorderSum/list" method="post">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <shiro:hasPermission name="sellOrderSum:add"><a onclick="article_add('添加订单汇总','/sellorderSum/add')" href="javascript:;" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加订单汇总</a></shiro:hasPermission>
        </span>
        ...
        <span class="r">
            ...
            <shiro:hasPermission name="sellOrderSum:import"><a onclick="article_import('导入excel','/sellorderSum/excelUpload')" href="javascript:;" class="btn btn-secondary-outline  size-M radius"><i class="Hui-iconfont">&#xe645;</i> 导入excel文件</a></shiro:hasPermission>
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <c:forEach var="sellorderSum" items="${sellorderSums}">
                <tr class="text-c">
                    ...
                    <td>${sellorderSum.company}</td>
                    <td>${sellorderSum.customername}</td>
                    <td>${sellorderSum.onlinesum}</td>
                    <td>${sellorderSum.underlinesum}</td>
                    <td>${sellorderSum.freight}</td>
                    <td>${sellorderSum.discountprice}</td>
                    <td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" onClick="article_detail('详情','/sellorderSum/detail','${sellorderSum.sellordercode}')" href="javascript:;" title="详情"><i class="Hui-iconfont">&#xe715;</i></a>
                        <shiro:hasPermission name="sellOrderSum:update"><a style="text-decoration:none" class="ml-5" onClick="article_edit('编辑','/sellorderSum/update','${sellorderSum.sellordercode}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a></shiro:hasPermission>
                        <shiro:hasPermission name="sellOrderSum:delete"><a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${sellorderSum.sellordercode}')"  href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></shiro:hasPermission></td>
                </tr>
            </c:forEach>
            </tbody>
            <thead>
            <tr class="text-c" style="font-weight:bold;font-size:300px;">
                <th colspan="5" >价格统计：</th>
                <th>${onlineSums}</th>
                <th>${underlineSums}</th>
                <th>${freights}</th>
