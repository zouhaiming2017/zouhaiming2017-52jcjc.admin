...
<article class="page-container">
    <form class="form form-horizontal" id="form-article-update" method="post">
        <div class="row cl">
            <%--<label class="form-label col-xs-4 col-sm-2">用户ID：</label>--%>
            <div class="formControls col-xs-8 col-sm-9">
                <input readonly="readonly" type="hidden" class="input-text" value="${user.id}" placeholder="" id="id" name="id">
            </div>
        </div>
    ...
    function article_save_submit(){
        ...
        $("#form-article-update").validate({
            ...
            submitHandler:function(form){
                    var html = document.getElementById('form-article-update').value;
                    var newpassword = document.getElementById('newpassword').value;
                    var newpassword2 = document.getElementById('newpassword2').value;
                    if (newpassword2 != newpassword) {
                        layer.alert('两次密码不一致！');
                    }else {
                        html.action="/user/updatePassword";
