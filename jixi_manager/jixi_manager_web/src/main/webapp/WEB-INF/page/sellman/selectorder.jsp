...
<head>
    ...
    <![endif]-->
    <title><strong>${sellmanname}</strong>所有订单列表</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span><strong>${sellmanname}</strong>所有订单管理 <span class="c-gray en">&gt;</span> <strong>${sellmanname}</strong>所有订单列表
    <%--<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a>--%>
</nav>
<div class="page-container">
    <div class="text-c">
        <form action="/sellman/selectorder" method="post">
            <button onclick="removeIframe()" class="btn btn-primary radius">关闭选项卡</button>
            <input type="hidden" name="sellmanname" value="${sellmanname}"/>
            ...
            <button name=""  class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜${sellmanname}订单</button>
        </form>
    </div><br/>
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="r">
            <shiro:hasPermission name="sellOrderSum:setCommission"><a onclick="setCommission('设置提成','/sellman/setCommission')" href="javascript:;" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 设置提成</a></shiro:hasPermission>
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <c:forEach var="sellorderSum" items="${sellorderSums}">
                <tr class="text-c">
                    ...
                    <td>${sellorderSum.commission}</td>
                    <td class="td-status">
                        <c:if test="${sellorderSum.allpaystatue==null||sellorderSum.allpaystatue==0}">
                            <span class="label label-defaunt radius">未结清</span>
                        </c:if>
                        <c:if test="${sellorderSum.allpaystatue==1}">
                            <span class="label label-success radius">已结清</span>
                        </c:if>
                    </td>
                    <td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5" onClick="article_detail('详情','/sellorderSum/detail','${sellorderSum.sellordercode}')" href="javascript:;" title="详情"><i class="Hui-iconfont">&#xe715;</i></a>
                        <shiro:hasPermission name="sellOrderSum:update"><a style="text-decoration:none" class="ml-5" onClick="article_edit('编辑','/sellorderSum/update','${sellorderSum.sellordercode}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a></shiro:hasPermission></td>
                </tr>
            </c:forEach>
            </tbody>
            <thead>
...
//        var params = {"sellordercodes": checkedList.toString()};
        document.location.href="/sellman/exportExcelAsSellman?sellordercodes="+checkedList.toString();
    ...
    /*添加*/
    function setCommission(title,url){
        var index = layer.open({
            type: 2,
            title: title,
            content: url
